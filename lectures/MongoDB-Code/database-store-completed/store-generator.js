...
MongoClient.connect("mongodb://localhost:27017/", function(err, client) {
  ...
  db = client.db('store');
  db.collection("products").insertOne({a:1}, function(err, result){
	  if(err) throw err;
	  ...
	  db.collection("products").drop(function(err, delOK){
		...
		db.collection("products").insertMany(productList, function(err, result){
			if(err) throw err;
			console.log("Successfuly inserted " + result.insertedCount + " products.")
			process.exit();
		})
	  });
  });
